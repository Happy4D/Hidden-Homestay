...
def run():
    ...
    with sync_playwright() as p:
        ...
        def mock_api(busy=None, ref='HH-TEST1'):
            def handler(route):
        ...
        check('dark theme restored (no white body)', page.evaluate("getComputedStyle(document.body).backgroundColor") not in ('rgb(255, 255, 255)', 'rgb(247, 245, 255)'), page.evaluate("getComputedStyle(document.body).backgroundColor"))
        ...
        print('== HERO SLIDER ==')
        check('12 slides built', page.locator('#showcaseFrame .slide').count() == 12, page.locator('#showcaseFrame .slide').count())
        ...
        check('EN mode: tag chips still shown on all 12 rooms', page.locator('.rooms .room-tags').count() == 12, page.locator('.rooms .room-tags').count())

        print('== ROOM CARD -> BOOKING (preselected) ==')
        ...
        check('check-in select is bold + solid dark (visible)', page.locator('#bkIn').evaluate("el => { const s = getComputedStyle(el); return s.fontWeight === '700' && s.backgroundColor !== 'rgba(0, 0, 0, 0)'; }"))
        page.click('#actNext1')
        page.wait_for_timeout(500)
        check('Vintage Room already preselected in step 2', 'sel' in (page.locator('#roomGrid .room-card[data-room="vintage"]').get_attribute('class') or ''))
        check('next enabled without re-picking the room', not page.locator('#actNext2').is_disabled())
        ...
        check('11 house rules displayed', page.locator('#termsList li').count() == 11, page.locator('#termsList li').count())
